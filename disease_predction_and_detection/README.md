Disease_predction_and_detection
==============================
...
```bash
conda create python=3.8 -n disease_predction_and_detection
```
2 - <a name="step-2">Activate the conda environment</a>

```bash
conda activate disease_predction_and_detection
```
...
If for some reason you get an error similar to bellow, is because the DATABASE_URL is configured to `postgres:///disease_predction_and_detection` and because of it the generated `DATABASES` settings are configured to connect on PostgreSQL using the socket mode.
...
LINE 1: CREATE DATABASE disease_predction_and_detection WITH ENCODING 'UTF-8' OWNER "";
                                                             ^
ERROR:  syntax error at or near ";"
LINE 1: GRANT ALL PRIVILEGES ON DATABASE disease_predction_and_detection TO ;
```
...
ERROR:  database "disease_predction_and_detection" already exists
GRANT
```
...
```bash
sudo -u postgres dropdb -U postgres --if-exists disease_predction_and_detection
...
```bash
dropdb --if-exists disease_predction_and_detection
